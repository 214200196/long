...
<html lang="zh-CN">
  <head>
	...
	<title>会员中心-<?php echo ($user["name"]); ?></title>
...
<!--     <ul class="mp clearfix">
    ...
    <ul class="nav nav-pills">
        <li role="presentation" class="">
            <a  href="<?php echo U('visitor',array('uid'=>$_GET['uid'],'menue'=>0));?>"><i class="glyphicon glyphicon-book" style="margin-right:10px;"></i>Ta发布的图书</a>
         <li role="presentation" class="">
            <a  href="<?php echo U('visitor',array('uid'=>$_GET['uid'],'menue'=>1));?>"><i class="glyphicon glyphicon-heart" style="margin-right:10px;"></i>Ta关注的图书</a>
        </li>
    </ul>
        ...
        <ul class="mp clearfix" style="margin-bottom:10px;">
            <li class="l mp-item">
              <span class="mp-atag">
                <b class="mp-title">今日访问<span><?php echo ($user["today_visitor"]); ?></span></b>
              </span>
            </li>
            <li class="l mp-item">
              <span class="mp-atag">
                <b class="mp-title">总访问<span><?php echo ($user["count_visitor"]); ?></span></b>
              </span>
            </li>
        </ul>
  ...
  <div class="r space-main">
    ...
    <div class="family">
        <h1 class="family-hd">Ta关注的图书</h1>
    </div>
    <div class="course-tool-bar clearfix">
        <div class="tool-left l">
            <a class="sort-item active"><span class="glyphicon glyphicon-th-list"></span>&nbsp;&nbsp;关注列表</a>
        </div>
        <div class="tool-right r">
            <span class="tool-item total-num">共<b></b>个课程</span>
            <span class="tool-item tool-pager">
                <span class="pager-num"><b class="pager-cur"><?php echo ((isset($_GET['p']) && ($_GET['p'] !== ""))?($_GET['p']):1); ?></b>/<em class="pager-total"><?php echo ((isset($totalPage) && ($totalPage !== ""))?($totalPage):1); ?></em>&nbsp;&nbsp;</span>
                <?php if($_GET['p'] != 1): ?><a href="<?php echo U('visitor',array('uid'=>$_GET['uid'],'p'=>$_GET['p']-1));?>" class="glyphicon glyphicon-arrow-left"></a>&nbsp;&nbsp;<?php endif; ?>
                <?php if($_GET['p'] < $totalPage): ?><a href="<?php echo U('visitor',array('uid'=>$_GET['uid'],'p'=>$_GET['p']+1));?>" class="glyphicon glyphicon-arrow-right"></a><?php endif; ?>
            </span>
        </div>
    </div>

      <ul class="follow-list">
        <?php if(is_array($followResult)): foreach($followResult as $key=>$v): ?><li data-id="<?php echo ($v["id"]); ?>" >
                  ...
                  <div class="box-right">
                      ...
                      <?php  $getMiniContentId = M('content_category')->where(array('bid'=>$v['id'],array('content_id'=>array('NEQ',0))),'AND')->order("content_id ASC")->field("content_id")->limit(1)->find(); $this->getMiniContentId = $getMiniContentId; ?>
                          ...
                          <?php if(in_array($v['id'],$followStatus)): ?><a class="btn btn-default" style="padding:6px 20px;" role="button">已关注&nbsp;<strong style="color:red;" class="glyphicon glyphicon-heart"></strong></a>
                          <?php else: ?>
                            <a href="<?php echo U('addFollow',array('bid'=>$v['id']));?>" class="btn btn-default" style="padding:6px 20px;" role="button">关注&nbsp;<strong style="color:red;" class="glyphicon glyphicon-heart-empty"></strong></a><?php endif; ?>

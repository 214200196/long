    ...
    <div class="user-info">
        ...
        <ul class="user-lay">
            <li class="mynick-name">
                 <span class="user-name"><?php echo ($visitor["name"]); ?></span> 
            </li>
            <li>
...
<!--     <ul class="mp clearfix">
    ...
    <ul class="nav nav-pills">
        ...
        <ul class="mp clearfix" style="margin-bottom:10px;">
            <li class="l mp-item">
              <span class="mp-atag">
                <b class="mp-title">今日访问<span><?php echo ((isset($visitor["today_visitor"]) && ($visitor["today_visitor"] !== ""))?($visitor["today_visitor"]):0); ?></span></b>
              </span>
            </li>
            <li class="l mp-item">
              <span class="mp-atag">
                <b class="mp-title">总访问<span><?php echo ((isset($visitor["count_visitor"]) && ($visitor["count_visitor"] !== ""))?($visitor["count_visitor"]):0); ?></span></b>
              </span>
            </li>
        </ul>
    ...
    <div class="course-tool-bar clearfix">
        ...
        <div class="tool-right r">
            ...
            <span class="tool-item tool-pager">
                ...
                <?php if($_GET['p'] != 1): ?><a href="<?php echo U('follow',array('menue'=>1,'p'=>$_GET['p']-1));?>" class="glyphicon glyphicon-arrow-left"></a>&nbsp;&nbsp;<?php endif; ?>
                <?php if($_GET['p'] < $totalPage): ?><a href="<?php echo U('follow',array('menue'=>1,'p'=>$_GET['p']+1));?>" class="glyphicon glyphicon-arrow-right"></a><?php endif; ?>
            </span>
        </div>
    </div>
        ...
        <?php if(is_array($followResult)): foreach($followResult as $key=>$v): ?><li data-id="<?php echo ($v["id"]); ?>" >
                  ...
                  <div class="box-right">
                      ...
                      <?php  $getMiniContentId = M('content_category')->where(array('bid'=>$v['id'],array('content_id'=>array('NEQ',0))),'AND')->order("content_id ASC")->field("content_id")->limit(1)->find(); $this->getMiniContentId = $getMiniContentId; ?>
                          <a href="<?php echo U('Books/index',array('bid'=>$v['id'],'content_id'=>$getMiniContentId['content_id']));?>" class="beginstudy" data-title="" target="_blank">查看</a>
                          <a style="background-color:#FF7744;" href="<?php echo U('delFollow',array('bid'=>$v['id']));?>" class="beginstudy" id="delfollow" data-title="">取消关注</a>

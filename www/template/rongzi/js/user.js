...
/* 检查邮箱 */
function checkEmail(email,ajxurl) {
...
/* 检查用户名 */
function checkUsername(username,ajxurl) {
    var submit_disabled = false;
    if (username != "") {
...
/* 检查用户昵称 */
function checkNiname(username,ajxurl) {
    var submit_disabled = false;
    ...
    var patrn = /^([a-zA-Z0-9_]|[\u4E00-\u9FA5]){2,15}$/;
    if (username == "") {
        document.getElementById('niname_notice').innerHTML = nikename_msg;
        submit_disabled = true;
    }
    else if (!patrn.exec(username)) {
        document.getElementById('niname_notice').innerHTML = nikename_msg;
        ...
        $.ajax({
            type: "get",
            url: ajxurl,
            data: '&q=check_niname&niname='+ username,
            success: function (result) {
                if (result == 1) {
					
                    document.getElementById('niname_notice').innerHTML = msg_can_reg;
                    submit_disabled = false;
                }
                else {
                    document.getElementById('niname_notice').innerHTML = nikename_msg_exist;
                    submit_disabled = true;
                }
            },
            cache: false
...
var msg_can_reg = "<img src='/template/rongzi/images/42.gif' />";
var username_msg = ' 请输入3-15位字符';
var nikename_msg='请输入1-15为字符';
var username_msg_exist = " 用户名已经存在,请重新输入";
var nikename_msg_exist= " 昵称已经存在,请重新输入";

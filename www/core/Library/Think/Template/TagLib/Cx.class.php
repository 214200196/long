...
class Cx extends TagLib {
	...
	public function _lists($tag, $content){
		$item = $this->autoBuildVar ( $tag ['item'] );
		$module=$tag['module']."Class";
		$function=$tag['function'];
		foreach($tag as $_key =>$_val) {
			if ($_key !="module"&&$_key!="function"&&$_key!="item"){
				if ($_val == "request"){
					$_data[] = "'{$_key}'=>isset(\$_REQUEST['{$_key}'])?\$_REQUEST['{$_key}']:''";
				}elseif ('$' == substr ( $_val, 0, 1 )){
					$val=$this->autoBuildVar ( substr ( $_val, 1 ) );
					$_data[] = "'{$_key}'=>{$val}";
		...
		$parseStr = '<?php ' . $item . ' = '.$module.'::'.$function.'('. $data .')'. '; ?>';
		return $parseStr;
	}
	/**
	 * define标签解析
